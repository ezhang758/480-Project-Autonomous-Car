...
def imread(img_path, load_fast=False):
    img = cv2.imread(img_path)
    if not load_fast and img is not None and len(img.shape) == 3:
        img = np.array(img[:, :, ::-1])
    return img
...
def generate_xz_y_lin(points_df):
    xz_y_lin = LinearRegression()
    X = points_df[['x', 'z']].to_numpy()
    y = points_df['y'].to_numpy()
    xz_y_lin.fit(X, y)
    pickle.dump(xz_y_lin, open("xzy_slope.sav", 'wb'))
...
def generate_img_coords(string, camera_matrix=camera_matrix):
    ...
    # img_x = coords_before_perspective[:, 0]
    # img_y = coords_before_perspective[:, 1]
    # img_z = coords_before_perspective[:, 2] # z = Distance from the camera
...
def visualize(img, coords, camera_matrix=camera_matrix):
    # define the reference point position, found from https://www.kaggle.com/zstusnoopy/visualize-the-location-and-3d-bounding-box-of-car,
    x_l = 1.02
    y_l = -0.80
    z_l = 2.31
...
def generate_train_test_data(path, train_test_split_ratio, random_state):
    ...
    generate_xz_y_lin(points_df)


def preprocess_main(batch_size, path, train_test_split_ratio=0.01, random_state=42):
    ...
    generate_train_test_data(path, train_test_split_ratio, random_state)
...
if __name__ == "__main__":
    ...
    preprocess_main(args.batch_size, args.path, args.train_test_split_ratio, args.random_state)
