...
def calculate_mean_average_precision_helper(img_df, rot_thre_list, tran_thre_list, true_object_list, pred_object_list):
    ...
    #         rot_thre = 120
    #         tran_thre = 1
    #         img_new0 = img_df[img_df['rot_dist'] < rot_thre]
    # #         no_object_smaller_rot_dist = len(img_new0.index)
    #         img_new0 = img_new0[img_new0['trans_dist'] < tran_thre]
    #         no_TP = len(img_new0.index)
    #         no_FP = len(pred_object_list) - no_TP
    #         precision = no_TP/total_no_of_pred_truth_positives
    #         recall = no_TP/total_no_of_ground_truth_positives
...
if __name__=="__main__":
    pred_df = pd.read_csv('MyUNet_vggnet_predictions.csv')
    pred_df.head(5)
    names_true = ['id', 'yaw_true', 'pitch_true', 'roll_true', 'x_true', 'y_true', 'z_true']
    names_pred = ['yaw', 'pitch', 'roll', 'x', 'y', 'z', 'confidence']
    
    inp = pred_df['PredictionString'][0]
    print('Example input:\n', inp)
    print()
    print('Output:\n', str2coords(inp, names=names_true))
    
    inp = pred_df['NewPredictionString'][0]
    print('Example input:\n', inp)
    print()
    print('Output:\n', str2coords(inp, names=names_pred))
